...
RUN apt-get update -y && \
    ...
    # FFmpeg. Upstream pinned autobuild-2024-07-30-14-10 (see #347, an illegal-instruction
    # report). yt-dlp/FFmpeg-Builds PRUNES autobuild releases after ~a week, so that URL now
    # 404s and every build from this Dockerfile fails at the tar step. The `latest` tag is a
    # permanent, always-present release, so use it instead of a pin that is guaranteed to rot.
    # `curl --fail` so a dead URL errors here instead of saving an HTML error page and
    # failing later with an inscrutable "tar: Error is not recoverable".
