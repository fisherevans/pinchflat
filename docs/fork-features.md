...
## Metrics export (agentless, pluggable)

Pushes behavioral/operational metrics, events, and error logs straight to a hosted backend over HTTP with no local agent. Off by default; enabled by setting `DATADOG_API_KEY` (mirrors the `ENABLE_PROMETHEUS` toggle). The export layer is pluggable - a backend implements `Pinchflat.Metrics.Backend` and is selected by `:metrics_backend`, so Grafana/OTLP can be added later without touching instrumentation. Full reference: [`metrics.md`](metrics.md).

Architecture mirrors the runner-injection pattern: a `Pinchflat.Metrics` facade (`count`/`gauge`/`event`/`log`, near-free no-op when disabled, failure-isolated so a metric can never crash or block a worker) feeds a `Buffer` GenServer that aggregates (counts summed by name+tags, gauges last-value) and flushes to the configured backend every ~15s. `Backends.Datadog` ships to Datadog's HTTP intake APIs (series/events/logs) via the supervised Finch pool; `Backends.Noop` is the default.

Instrumented behavior: downloads (`download.completed`/`.recovered`/`.failed` with the failure tagged by a classified `reason` - `auth_needed`/`members_only`/`unavailable`/`rate_limited`/`forbidden`/`other` - plus a failure event and `download.bytes`); a `jobs.executing` concurrency gauge per Oban queue (a gated `telemetry_poller`); per-queue job success/failure/duration from Oban's own telemetry; `index.new_items`; retention culled/evicted counts + bytes freed; and yt-dlp self-update success/failure. A gated `:logger` handler forwards `warning`/`error` log lines (with their media/source metadata) to Datadog Logs, loop-guarded against the metrics subsystem's own logs. `docs/metrics.md` lists recommended monitors (re-auth needed, download-failure-rate, nothing-downloading, update-failing).
